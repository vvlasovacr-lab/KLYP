...
// НОМЕР СБОРКИ.
//
// Telegram кеширует страницу мини-аппа у себя и держит её после выкатки:
// на сервере уже новая версия, а в телефоне открывается вчерашняя. Это
// стоило нам двух дней разбирательств — со стороны выглядело так, будто
// работа не сделана.
//
// Заголовок no-store помогает не всегда: Telegram решает сам. Надёжно
// работает другое — менять адрес. Номер подставляется в ссылку, которую
// шлёт бот, и каждая выкатка становится для Telegram новой страницей.
//
...
export const VERSION = (
	...
	''
).slice(0, 7) || String(Date.now()).slice(-7);
...
export const config = {
	...
	render: {
		...
		// Готовый ролик отдаём меньше кадра, чтобы файл весил как настоящий
		// рилс. Доля обязана давать целое число точек: 0.667 от 1920 — это
		// 1280,64, и рендер падает на дробной высоте. Три четверти делятся
		// начисто: 1080×1920 → 810×1440.
		deliverScale: Number(opt('DELIVER_SCALE', 0.75)),
		...
		// Сколько вкладок браузера рендерят кадры одновременно.
		// Пусто — решает сам Remotion, и это правильный выбор: он умеет
		// читать настоящий лимит контейнера, а Node показывает ядра всей
		// машины. На Railway это 48 против реальных двух.
		concurrencyPerRender: set('RENDER_CONCURRENCY_PER_JOB'),
		// Порт для внутреннего сервера Remotion. Обязан отличаться от
		// нашего: иначе браузер просит кадры у приложения вместо рендера.
		remotionPort: Number(opt('REMOTION_PORT', Number(opt('PORT', 3000)) + 771)),
		// Потолок кэша распакованного видео, байты. Полгигабайта хватает
		// на окно кадров и не выдавливает всё остальное из памяти.
		videoCacheBytes: Number(opt('RENDER_VIDEO_CACHE_BYTES', 512 * 1024 * 1024)),
		// Отрисовка кадра на процессоре вместо видеокарты. Спасает там,
		// где карты нет вовсе, но забирает процессор у распаковки видео:
