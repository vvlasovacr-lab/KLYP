...
const trimPauses = async (from, to) => {
	...
	// Картинку отбираем по времени, а не режем кусками с последующей
	// склейкой: склейка опирается на метки времени в файле, а они у
	// снятого телефоном видео бывают неровными — и тогда обрезка молча
	// не срабатывает.
	const ranges = parts
	...
	// ЗВУК СОБИРАЕМ ОТДЕЛЬНО И СО СКРУГЛЕНИЕМ СТЫКОВ.
	//
	// Тот же отбор по времени резал звук встык. На слух это щелчок: на
	// одном из стыков громкость прыгала с тихой комнаты до середины
	// громкого слова за миллисекунду. Живая речь так не начинается —
	// она разгоняется за десятки миллисекунд, и ухо отличает одно от
	// другого мгновенно.
	//
	// Поэтому каждый кусок берётся отдельно и на своих краях гасится за
	// пятнадцать миллисекунд. Для речи это неслышно, а ступеньки на
	// стыке не остаётся.
	const FADE = 0.015;

	const pieces = [...parts, [at, null]];
	const cuts = pieces
		.map(([x, y], i) => {
			// Гасим только настоящие стыки. Начало первого куска и конец
			// последнего — это края ролика, а не склейка: затухание там
			// съело бы первое слово и оборвало последнее.
			const steps = [
				`[0:a]atrim=start=${x.toFixed(3)}${y === null ? '' : `:end=${y.toFixed(3)}`}`,
				'asetpts=PTS-STARTPTS',
			];

			if (i > 0) steps.push(`afade=t=in:st=0:d=${FADE}`);
			if (y !== null) {
				const out = Math.max(0, y - x - FADE);
				steps.push(`afade=t=out:st=${out.toFixed(3)}:d=${FADE}`);
			}

			return `${steps.join(',')}[a${i}]`;
		})
		.join(';');

	const glue = pieces.map((_, i) => `[a${i}]`).join('');
	...
	await new Promise((resolve, reject) => {
		const ff = spawn('ffmpeg', [
			...
			`${cuts};${glue}concat=n=${pieces.length}:v=0:a=1[a]`,
			'-map', '[v]', '-map', '[a]',
