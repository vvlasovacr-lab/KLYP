...
//
// Вертикальное видео ложится само. С горизонтальным раньше было два
// пути, и оба плохие.
//
// Полная обрезка до вертикали выбрасывает две трети ширины: если
// человек снят средним планом, остаётся лицо в упор. Клиент назвал это
// «просто приблизил», и он прав — картинка раздута почти вдвое.
//
// Полоса во всю ширину даёт обратное: резкая картинка занимает треть
// экрана, человек в ней мелкий, вокруг размытые поля.
//
// Середины не было. Теперь она главная: режем мягко, до четырёх пятых,
// ставим кадр сверху, а низ уходит под размытый фон и текст. Человек
// остаётся крупным, но не в упор, и внизу появляется место под
// субтитры — там, где раньше они лежали прямо на лице.
const SOFT = 0.8; // ширина к высоте у мягкого кадра: 4:5
...
const fitFilter = ({size, preview, frame}) => {
	...
	// Размытая копия во весь кадр — общая подложка для всех способов,
	// кроме полной обрезки.
	const back =
		`[bg]scale=${W}:${H}:force_original_aspect_ratio=increase,` +
		`crop=${W}:${H},boxblur=28:2[b];`;

	if (frame?.fit === 'полоса') {
		// Лента во всю ширину — для записи экрана и таблиц, где важен
		// каждый край. Поднята выше середины: по центру она ложится ровно
		// туда, где идут субтитры.
		return `split[bg][fg];${back}[fg]scale=${W}:-2[f];[b][f]overlay=(W-w)/2:(H-h)*0.34`;
	}
	...
	// Полная обрезка оставлена на случай, когда человек в кадре мелкий и
	// далеко: там приблизить — единственный выход.
	const tight = frame?.fit === 'крупно';
	const ratio = tight ? 9 / 16 : SOFT;

	let cropW = Math.round((size.h * ratio) / 2) * 2;
	if (cropW > size.w) cropW = Math.round(size.w / 2) * 2;

	const x = Math.round(Math.min(Math.max(focus * size.w - cropW / 2, 0), size.w - cropW));
	const cut = `crop=${cropW}:${size.h}:${x}:0`;

	if (tight) return `${cut},scale=${W}:${H}`;

	// Мягкий кадр: вписываем по ширине и ставим сверху, оставляя низ под
	// текст. Высота считается сама из пропорции — она заведомо меньше
	// кадра, иначе это была бы обычная обрезка.
	return (
		`split[bg][fg];${back}` +
		`[fg]${cut},scale=${W}:-2[f];` +
		`[b][f]overlay=(W-w)/2:${Math.round(H * 0.08)}`
	);
};
