...
// ЗНАКИ ПРЕПИНАНИЯ В СУБТИТРАХ.
//
// В кадре их не должно быть вовсе. Запятая и точка нужны читателю, чтобы
// разобрать длинный текст глазами; субтитр же идёт под голос, паузы в
// нём расставлены самой речью, а знак только висит хвостом у слова и
// выдаёт машинную расшифровку. В роликах, на которые мы равняемся, нет
// ни одного.
//
// Вопрос и восклицание оставляем: они не про паузу, а про интонацию, и
// без них «серьёзно?» превращается в «серьёзно».
//
// Чистим при сборке кадра, а не в самой расшифровке: на экране вычитки
// знаки помогают человеку читать и понимать, где кончается мысль.
const TAIL = /[.,;:]+$/;

const noPunctuation = (text) => String(text).replace(TAIL, '');
...
const toChunks = (scenes) =>
	scenes
		.map((scene) => ({
			...
			words: (scene.words ?? [])
				.map((word) => ({
					text: noPunctuation(word.word ?? word.text ?? ''),
